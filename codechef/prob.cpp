...
int main(){
    ...
    for(int i = 0; i<n; i++){ // iteratring from 0th row to n-1 th row
        for(int j = 0; j<m; j++){ // iterating from 0th to m-1 index of each row
            cin>>arr[i][j]; 
        }
    }
    ...
    for(int i = 0; i<8; i++){ // checking all 8 direction
        int a = x + dir[i][0], b = y + dir[i][1]; // updating x & y co-ordinates
        if(a >= 0 && a < n && b >= 0 && b < m && arr[a][b] == '.'){ // if a & b, both are inside matrix and contain '.' then answer is no, so update value of possible to false 
            possible = false;
            break;
        }
    }
